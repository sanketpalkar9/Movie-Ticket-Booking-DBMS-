...
export const PictureQualitySelector = ({ hallData, setHallData }) => {
  ...
  const checkedColor = (val) => {
    return {
      backgroundColor: val === userAns ? "var(--logo)" : "",
      border: val === userAns ? "2px solid transparent" : "",
      // color: val === userAns ? "#e6e6e6" : "",
  ...
  const showtimeOptions = newHallData.map((show) => {
    ...
    return (
      ...
      >
        ...
        <p className="form-show-price">{`INR ${show.price_per_seat}`}</p>
      </div>
    );
  });
  ...
  return (
    <div>
      <form>
        <div className="form-item-heading">Select Quality and Time</div>
